...
def Trans(data, trans, lr):
    if bool(len(trans)):
        dataf = torch.rfft(data, 2, normalized=True,onesided=False)
        dataf_t = fft_prod(dataf, conj(trans))
        data_t = torch.irfft(dataf_t, 2, normalized=True,onesided=False)
...
def CalTrans(vars, regs, lambda_):
    varf = torch.rfft(vars, 2, normalized=True, onesided=False)
    regf = torch.rfft(regs, 2, normalized=True, onesided=False)
